 ...
 * Hardware : ESP32-C3 SuperMini + L9637D + GPS (UART0 หรือ UART2)
 * 
 * โหมด:
 *   NORMAL MODE — บันทึก CSV ใน LittleFS + WiFi AP dashboard
 *   TRACK MODE  — บันทึก CSV + ส่ง UDP realtime → PC server ข้างสนาม
 *                 วัด lap time อัตโนมัติด้วย start/finish line (GPS)
 *
 * วงจร:
 *   L9637D Pin1 RX → GPIO21  |  Pin4 TX → GPIO20  (K-Line)
 *   GPS TX         → GPIO3   |  GPS RX  → GPIO2   (UART0 / Neo-6M)
 *   GPIO9 (BOOT)   → กดเพื่อ toggle Track Mode
 *
 * WiFi:
 *   NORMAL: AP mode  SSID="YamahaLog"  pass="yamaha1234"
 *   TRACK : STA mode เชื่อม hotspot มือถือ แล้วส่ง UDP → PC
 *
 * Universal CSV format (RaceRender + MoTeC + AiM + Excel compatible):
 *   บรรทัดแรก: # RaceRender Data
 *   Time(s), RPM, KPH, Throttle, Gear, ...
 *
 * Library ที่ต้องติดตั้ง (Library Manager):
 *   - TinyGPSPlus  (by Mikal Hart)
 *   - ArduinoJson  (by Benoit Blanchon)  ← ใช้ส่ง UDP JSON
...
#define GPS_TX_PIN       2    // GPS RX → ESP32 TX (ไม่ค่อยใช้)
#define GPS_BAUD         9600

#define MODE_BTN_PIN     9    // BOOT button — toggle track mode

#define LOG_INTERVAL_MS  100  // track mode: 10Hz | normal: 500ms
...
// WiFi STA (Track mode) — แก้ให้ตรงกับ hotspot มือถือ
#define STA_SSID         "MyPhone"
#define STA_PASS         "phone1234"

// PC Server (Track mode)
#define SERVER_IP        "192.168.x.x"   // ← IP ของ PC ข้างสนาม
#define SERVER_UDP_PORT  5005

// Start/Finish line GPS coordinate (ตั้งก่อนออกสนาม)
// ตัวอย่าง: สนาม PTT Bira Circuit
#define SF_LAT           12.6822
#define SF_LON           101.0456
#define SF_RADIUS_M      15.0    // รัศมี trigger (เมตร)
...
struct LapData {
  uint8_t  lapNum     = 0;
  uint32_t lapStart   = 0;    // millis
  uint32_t lastLapMs  = 0;    // ms ของ lap ก่อน
  uint32_t bestLapMs  = 0;    // best lap ms
  bool     inSFZone   = false;
  float    maxRPM     = 0;
  float    maxSpeed   = 0;
};
...
// คำนวณระยะห่างสองจุด GPS (เมตร) — Haversine
...
void checkLapTrigger() {
  ...
  double dist = gpsDistance(bike.lat, bike.lon, SF_LAT, SF_LON);
  bool inZone = dist < SF_RADIUS_M;
  ...
  if (inZone && !lap.inSFZone) {
    ...
    if (lap.lapNum > 0) {
      ...
      Serial.printf("[LAP] #%d  %.3fs  best=%.3fs\n",
        lap.lapNum,
        lap.lastLapMs/1000.0f,
        lap.bestLapMs/1000.0f);
      // reset stats
...
uint32_t currentLapMs() {
  if (lap.lapNum == 0) return 0;
  return millis() - lap.lapStart;
}
...
int findNextIdx() {
  ...
  while(f){ String n=f.name();
    if(n.startsWith("log_")&&n.endsWith(".csv"))
      mx=max(mx,n.substring(4,7).toInt());
    f=root.openNextFile(); }
  return mx+1;
}

// Universal CSV header — compatible กับ RaceRender / MoTeC / AiM / Excel
bool logOpen(bool trackMode=false) {
  int idx=findNextIdx();
  if(idx>MAX_LOG_FILES) return false;
  char fname[24];
  snprintf(fname,sizeof(fname), trackMode?"/trk_%03d.csv":"/log_%03d.csv", idx);
  logFilename=fname;
  logFile=LittleFS.open(logFilename,"w");
  if(!logFile) return false;

  // Line 1: RaceRender marker
  logFile.println("# RaceRender Data");
  // Line 2: metadata comment
  logFile.printf("# Yamaha MT-15/R15V3  Mode:%s\n", trackMode?"TRACK":"NORMAL");

  // Line 3: Universal header
  // Time ใน seconds, ชื่อ column ตาม RaceRender spec
  // *OBD suffix = OBD interpolation ใน RaceRender
  // unit ใน [] สำหรับ MoTeC/AiM
  logFile.println(
    "Time,"          // s
    "RPM,"           // rpm  *OBD
    "KPH,"           // km/h
...
void logWrite() {
  ...
  float timeS = (millis()-logStartTime)/1000.0f;
  bool  gpsUpd = gps.location.isUpdated();  // GPS_Update flag

  char row[320];
  snprintf(row, sizeof(row),
    "%.3f,%d,%d,%d,%d,"          // Time,RPM,KPH,Throttle,Gear
    "%.7f,%.7f,%.1f,%.1f,%.1f,%d,"  // Lat,Lon,Alt,Heading,GPSspd,GPSupd
    "%d,%d,%d,%.1f,%.3f,%d,%.3f,%d,%d,%d,%d,"  // ECT,IAT,MAP,Batt,Inj,IGN,O2,ISC,VVA,ST,LT
    "%d,%.3f,%.3f,%.3f,"         // Lap,LapTime,LastLap,BestLap
    "%d",                         // DTC
    timeS,
    bike.rpm, bike.speed, bike.tps, bike.gear,
    bike.lat, bike.lon, bike.altitude, bike.heading, bike.gpsSpeed, gpsUpd?1:0,
    bike.ect, bike.iat, bike.map_kpa,
    bike.batt, bike.inj_ms, bike.ign_deg, bike.o2_v,
    bike.isc, bike.vva?1:0, bike.stFuel, bike.ltFuel,
    lap.lapNum, currentLapMs()/1000.0f,
    lap.lastLapMs/1000.0f, lap.bestLapMs/1000.0f,
    bike.dtcCount
  );
  logFile.println(row);
  logFile.flush();
...
void startAP() {
  ...
  Serial.printf("[WiFi] AP: %s → http://192.168.4.1\n", AP_SSID);
}
...
// ──────────────────────────────────────────────
//  WEB SERVER (Normal mode dashboard)
// ──────────────────────────────────────────────
void handleData() {
  char j[600];
  snprintf(j,sizeof(j),
...
h2{color:#777;font-size:.85em;margin:12px 0 6px;text-transform:uppercase;letter-spacing:.05em}
...
.c.hi .v{color:#3f6}.c.warn .v{color:#f33}.c.dim .v{color:#aaa}
.st{background:#181818;border:1px solid #222;border-radius:6px;padding:8px 12px;
    font-size:.8em;color:#8f8;margin-bottom:8px}
.dtbox{background:#2a0a0a;border:1px solid #633;border-radius:6px;
       padding:8px 12px;font-size:.8em;color:#f77;margin-bottom:8px;display:none}
.gps{background:#181818;border:1px solid #222;border-radius:6px;
     padding:8px 12px;font-size:.78em;color:#8af;margin-bottom:8px}
.ct{display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap}
.ct a{flex:1;min-width:80px;text-align:center;padding:9px;border-radius:7px;
...
function sc(id,cls){var e=document.getElementById(id);if(e){e.className='c '+cls}}
function refresh(){
  fetch('/data').then(r=>r.json()).then(d=>{
    ...
    sc('bcard',d.batt<11.5?'warn':'');
    sc('gcard',d.gear===0?'hi dim':'dim');
  });
}
setInterval(refresh,500);
</script>
</head><body>
<h1>🏍 Yamaha Datalog <span class='badge nm' id='mdbadge'>NORMAL</span></h1>
<h2>⚡ Engine</h2>
<div class='g3'>
  <div class='c'><div class='v' id='rpm'>-</div><div class='u'>RPM</div></div>
  <div class='c'><div class='v' id='spd'>-</div><div class='u'>km/h</div></div>
  <div class='c dim' id='gcard'><div class='v' id='gear'>-</div><div class='u'>Gear</div></div>
...
<div class='g3'>
  <div class='c'><div class='v' id='ect'>-</div><div class='u'>°C Coolant</div></div>
  <div class='c'><div class='v' id='iat'>-</div><div class='u'>°C Intake</div></div>
...
<div class='g3'>
  ...
  <div class='c' id='bcard'><div class='v' id='batt'>-</div><div class='u'>V Battery</div></div>
  <div class='c'><div class='v' id='o2'>-</div><div class='u'>V O2</div></div>
  <div class='c'><div class='v' id='isc'>-</div><div class='u'>ISC steps</div></div>
...
void switchMode(Mode newMode) {
  ...
  if (newMode == MODE_TRACK) {
    ...
    lap = LapData();   // reset lap data
    bool ok = startSTA();
    logOpen(true);     // สร้างไฟล์ trk_xxx.csv
    if (!ok) server.begin(); // fallback web server ถ้า STA ไม่ได้
  } else {
...
void loop() {
  ...
  // TesterPresent
  if (now - lastPresent > 1500) {
    sendTesterPresent(); lastPresent = now;
  }

  // อ่าน sensor
  uint16_t readInterval = (currentMode == MODE_TRACK) ? 100 : 200;
  if (now - lastRead > readInterval) {
    readAllSensors(); lastRead = now;
  }

  // Log interval
  uint16_t logInterval = (currentMode == MODE_TRACK) ? 100 : 500;
  if (loggingActive && now - lastLog >= logInterval) {
    logWrite(); lastLog = now;
  }

  // UDP ส่งทุก 100ms ใน track mode
  if (currentMode == MODE_TRACK && now - lastUDP >= 100) {
    udpSend(); lastUDP = now;
  }

  // Serial print
  if (now - lastPrint > 2000) {
    Serial.printf("[%s] RPM:%d SPD:%d TPS:%d GEAR:%d LAP:%d LT:%.2fs BEST:%.2fs GPS:%s\n",
      currentMode==MODE_TRACK?"TRACK":"NORMAL",
      bike.rpm, bike.speed, bike.tps, bike.gear,
